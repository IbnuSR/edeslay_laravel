            ...
            <div class="col-lg-8">
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                @if($surat->status == 'proses' && empty($surat->nomor_surat))
                                    <span class="badge bg-warning text-dark"><i
                                            class="fas fa-spinner fa-spin me-1"></i>Menunggu</span>
                                    ...
                                    @foreach($docs as $doc)
                                        <div class="col-md-4 mb-3">
                                            <div class="border rounded p-3 text-center bg-light">
                                                ...
                                                <a href="{{ asset($doc) }}" target="_blank" class="btn btn-sm btn-outline-primary">
                ...
                <div class="card shadow-sm">
                    ...
                    <div class="card-body">
                        @if($jenis == 'domisili')
                            <div class="row">
                                <div class="col-md-6"><label class="text-muted small">Tempat Tinggal</label>
                                    <p>{{ $surat->tempat_tinggal ?? '-' }}</p>
                                </div>
                                <div class="col-md-6"><label class="text-muted small">Keperluan</label>
                                    <p>{{ $surat->keperluan ?? '-' }}</p>
                                </div>
                            </div>
                        @elseif($jenis == 'sktm')
                            <div class="row">
                                <div class="col-md-4"><label class="text-muted small">Tanggungan</label>
                                    <p>{{ $surat->jumlah_tanggungan ?? '-' }} orang</p>
                                </div>
                                <div class="col-md-4"><label class="text-muted small">Status Ekonomi</label>
                                    <p>{{ $surat->status_ekonomi ?? '-' }}</p>
                                </div>
                                <div class="col-md-4"><label class="text-muted small">Tujuan</label>
                                    <p>{{ $surat->tujuan_skmt ?? '-' }}</p>
                                </div>
                            </div>
                        @elseif($jenis == 'kelahiran')
                            <div class="row">
                                <div class="col-md-6"><label class="text-muted small">Nama Bayi</label>
                                    <p>{{ $surat->nama_bayi ?? '-' }}</p>
                                </div>
                                <div class="col-md-6"><label class="text-muted small">TTL Bayi</label>
                                    <p>{{ $surat->tempat_lahir_bayi ?? '-' }}, {{ $surat->tanggal_lahir_bayi }}</p>
                                </div>
                                <div class="col-md-6"><label class="text-muted small">Nama Ayah</label>
                                    <p>{{ $surat->nama_ayah ?? '-' }}</p>
                                </div>
                                <div class="col-md-6"><label class="text-muted small">Nama Ibu</label>
                                    <p>{{ $surat->nama_ibu ?? '-' }}</p>
                                </div>
                            </div>
                        @elseif($jenis == 'ktp')
                            <div class="row">
                                <div class="col-md-6"><label class="text-muted small">Jenis Permohonan</label>
                                    <p>{{ $surat->jenis_permohonan ?? '-' }}</p>
                                </div>
                                <div class="col-md-6"><label class="text-muted small">Alasan</label>
                                    <p>{{ $surat->alasan_permohonan ?? '-' }}</p>
                                </div>
                            </div>
                        @elseif($jenis == 'kematian')
                            <div class="row">
                                <div class="col-md-6"><label class="text-muted small">Nama Almarhum</label>
                                    <p>{{ $surat->nama_almarhum ?? '-' }}</p>
                                </div>
                                <div class="col-md-6"><label class="text-muted small">Tanggal Kematian</label>
                                    <p>{{ $surat->tanggal_kematian }}</p>
                                </div>
                                <div class="col-md-6"><label class="text-muted small">Sebab Kematian</label>
                                    <p>{{ $surat->sebab_kematian ?? '-' }}</p>
                                </div>
                                <div class="col-md-6"><label class="text-muted small">Hubungan Pelapor</label>
                                    <p>{{ $surat->hubungan_pelapor ?? '-' }}</p>
                                </div>
                            </div>
                        @else
            ...
            @if($surat->status == 'proses' && empty($surat->nomor_surat))
                ...
                <div class="text-center">
                    ...
                    <p class="text-muted small mb-4">
                        Pengajuan baru masuk dan belum diproses admin.
                    </p>
                ...
                <!-- MULAI -->
                <form
                    action="{{ route('admin.surat.update', ['jenis' => $jenis, 'id' => $surat->id]) }}"
                    method="POST"
                    class="mb-3"
                >
            ...
            @if($surat->status == 'proses' && !empty($surat->nomor_surat))
                ...
                <div id="stageWaiting">
                    ...
                    <div class="text-center">
                        ...
                        <span class="badge bg-info fs-6 px-3 py-2 mb-3">
                            🔵 Sedang Dikerjakan
                        </span>

                        <p class="text-muted small mb-4">
                            Surat sedang dalam proses finalisasi admin.
                        </p>
                    ...
                    <button
                        type="button"
                        class="btn btn-outline-danger w-100 py-2 mt-2"
                        onclick="showRejectForm2()"
                    >
                        ❌ Tolak Pengajuan
                    </button>
                ...
                >
                    ...
                    >
                        ...
                        <button
                            type="button"
                            class="btn btn-light border w-100 py-2 mt-2"
                            onclick="hideFinalizeForm()"
                        >
                            Batal
                        </button>
            ...
            @if($surat->status == 'selesai')
                ...
                <div class="text-center">
                    ...
                    @if($surat->file_surat_jadi)
                        ...
                        <a
                            href="{{ asset('storage/' . $surat->file_surat_jadi) }}"
                            target="_blank"
    ...
    >
        ...
        <div class="text-start mb-3">
            ...
            <input
                ...
                class="form-control"
                value="{{ $surat->nomor_surat }}"
                placeholder="403/DS/TK/001/2026"
            >
        ...
        @if($surat->metode_pengambilan == 'cetak_online')
            ...
            <div class="text-start mb-3">
                ...
                <small class="text-muted d-block mt-1">
                    File ini akan otomatis muncul di HP masyarakat.
                </small>
        ...
        @else
            ...
            <div class="text-start mb-3">
                ...
                <textarea
                    ...
                    rows="3"
                    class="form-control"
                    placeholder="Contoh: Surat siap diambil jam 08.00-14.00 di Loket Pelayanan."
                    required
                ></textarea>

                <small class="text-muted d-block mt-1">
                    Info ini akan ditampilkan ke pemohon di mobile.
                </small>
            ...
            >
                Batal
                        ...
                        @if($surat->status == 'selesai')
                            <div class="mt-3">
                                ...
                                <p class="mt-2 mb-0 small text-muted">No: {{ $surat->nomor_surat }}</p> @endif
                                @if($surat->file_surat_jadi)
                                <p class="mt-1 small text-success"><i class="fas fa-check"></i> PDF Terupload</p> @endif
                                @if($surat->keterangan_admin)
                                    <p class="mt-1 small text-primary"><i class="fas fa-info-circle"></i>
                                {{ $surat->keterangan_admin }}</p> @endif
                            </div>
                        @endif
                        ...
                        @if($surat->status == 'ditolak')
                            <div class="mt-3">
                                ...
                                <form action="{{ route('admin.surat.update', ['jenis' => $jenis, 'id' => $surat->id]) }}"
                ...
                <div class="card shadow-sm mt-3">
                    <div class="card-body py-3">
                        ...
                        <div class="small">
                            <p class="mb-1"><i class="fas fa-circle text-primary me-1" style="font-size:8px;"></i> Diterima:
                                {{ \Carbon\Carbon::parse($surat->created_at)->format('d/m H:i') }}</p>
                            @if($surat->updated_at != $surat->created_at)
                                <p class="mb-0"><i class="fas fa-circle text-success me-1" style="font-size:8px;"></i> Update:
                                    {{ \Carbon\Carbon::parse($surat->updated_at)->format('d/m H:i') }}</p>
                            @endif
                        </div>
                    </div>
                </div>
