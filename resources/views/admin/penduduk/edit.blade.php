...
<div class="main-container">
    ...
    <div class="top-bar">
        <div class="page-header">
            ...
            <div class="breadcrumb">Dashboard / Data Penduduk / Edit</div>
        </div>
    ...
    <div class="content-card">
        ...
        <form method="POST" action="{{ route('admin.penduduk.update', $penduduk) }}">
                ...
                <div class="form-grid">
                    <div class="form-group">
                        ...
                        <input type="text" name="nik" maxlength="16" pattern="\d{16}" 
                               ...
                               placeholder="16 digit angka" value="{{ old('nik', $penduduk->nik) }}" required>
                        @error('nik')
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="nama" 
                               ...
                               placeholder="Nama lengkap sesuai KTP" value="{{ old('nama', $penduduk->nama) }}" required>
                        @error('nama')
                            <div class="error-message">⚠️ {{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="date" name="tanggal_lahir" 
                               ...
                               value="{{ old('tanggal_lahir', $penduduk->tanggal_lahir ? $penduduk->tanggal_lahir->format('Y-m-d') : '') }}" required>
                        @error('tanggal_lahir')
                            <div class="error-message">⚠️ {{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="form-grid">
                    <div class="form-group full-width">
                        ...
                        <select name="status_keluarga" class="form-control @error('status_keluarga') error @enderror" required>
                            ...
                            <option value="Anggota" {{ old('status_keluarga', $penduduk->status_keluarga) == 'Anggota' ? 'selected' : '' }}>Anggota Keluarga</option>
                        </select>
                ...
                <div class="form-grid">
                    <div class="form-group">
                        ...
                        <select name="pendidikan" class="form-control @error('pendidikan') error @enderror" required>
                            ...
                            <option value="Tidak Sekolah" {{ old('pendidikan', $penduduk->pendidikan) == 'Tidak Sekolah' ? 'selected' : '' }}>Tidak/Belum Sekolah</option>
                            <option value="Belum Tamat SD" {{ old('pendidikan', $penduduk->pendidikan) == 'Belum Tamat SD' ? 'selected' : '' }}>Belum Tamat SD</option>
                            <option value="SD" {{ old('pendidikan', $penduduk->pendidikan) == 'SD' ? 'selected' : '' }}>Tamat SD/Sederajat</option>
                            <option value="SMP" {{ old('pendidikan', $penduduk->pendidikan) == 'SMP' ? 'selected' : '' }}>SLTP/Sederajat</option>
                            <option value="SMA" {{ old('pendidikan', $penduduk->pendidikan) == 'SMA' ? 'selected' : '' }}>SLTA/Sederajat</option>
                            <option value="D1" {{ old('pendidikan', $penduduk->pendidikan) == 'D1' ? 'selected' : '' }}>Diploma I/II</option>
                            <option value="D2" {{ old('pendidikan', $penduduk->pendidikan) == 'D2' ? 'selected' : '' }}>Diploma I/II</option>
                            <option value="D3" {{ old('pendidikan', $penduduk->pendidikan) == 'D3' ? 'selected' : '' }}>Diploma III</option>
                            <option value="S1" {{ old('pendidikan', $penduduk->pendidikan) == 'S1' ? 'selected' : '' }}>Strata I</option>
                            <option value="S2" {{ old('pendidikan', $penduduk->pendidikan) == 'S2' ? 'selected' : '' }}>Strata II</option>
                            <option value="S3" {{ old('pendidikan', $penduduk->pendidikan) == 'S3' ? 'selected' : '' }}>Strata III</option>
                        </select>
                        @error('pendidikan')
                            <div class="error-message">⚠️ {{ $message }}</div>
                        @enderror
                    ...
                    <div class="form-group">
                        ...
                        <select name="pekerjaan" class="form-control @error('pekerjaan') error @enderror" required>
                            ...
                            <option value="Belum Bekerja" {{ old('pekerjaan', $penduduk->pekerjaan) == 'Belum Bekerja' ? 'selected' : '' }}>Belum/Tidak Bekerja</option>
                            ...
                            <option value="Karyawan Swasta" {{ old('pekerjaan', $penduduk->pekerjaan) == 'Karyawan Swasta' ? 'selected' : '' }}>Karyawan Swasta</option>
                            <option value="Petani/Pekebun" {{ old('pekerjaan', $penduduk->pekerjaan) == 'Petani/Pekebun' ? 'selected' : '' }}>Petani/Pekebun</option>
                    ...
                    <div class="form-group" id="kawinTercatatGroup" style="display: {{ old('status_perkawinan', $penduduk->status_perkawinan) == 'Kawin' ? 'block' : 'none' }};">
                        <label>Kawin Tercatat</label>
                        <select name="kawin_tercatat" class="form-control @error('kawin_tercatat') error @enderror">
                            <option value="">Pilih</option>
                            <option value="Ya" {{ old('kawin_tercatat', $penduduk->kawin_tercatat) == 'Ya' ? 'selected' : '' }}>Kawin Tercatat</option>
                            <option value="Tidak" {{ old('kawin_tercatat', $penduduk->kawin_tercatat) == 'Tidak' ? 'selected' : '' }}>Kawin Tidak Tercatat</option>
                        </select>
                ...
                <div class="form-grid">
                    <div class="form-group full-width">
                        ...
                        <input type="text" name="dusun" 
                               ...
                               placeholder="Nama dusun" value="{{ old('dusun', $penduduk->dusun) }}">
                        @error('dusun')
                            <div class="error-message">⚠️ {{ $message }}</div>
                        @enderror
                    </div>
            ...
            {{-- BUTTONS: Hapus (Kiri) & Simpan (Kanan) --}}
            <div class="form-actions">
                {{-- Tombol Hapus Data --}}
                <a href="{{ route('admin.penduduk.delete', $penduduk) }}" class="btn btn-danger" onclick="return confirm('Anda akan diarahkan ke halaman konfirmasi hapus. Lanjutkan?')">
                    🗑️ Hapus Data
                </a>
